-- MobileFinder initial schema (subset focused for MVP)
create extension if not exists postgis;

-- Roles are managed via auth.users; profiles stores role and display info
...
-- Stores: sellers can manage their stores; admins can do all (assume admin check via jwt role claim)
...
-- Store products: seller owns store
...
-- Reservations: user owns record; store seller can read for their store
...
-- Reviews: user owns; sellers can read for their store
